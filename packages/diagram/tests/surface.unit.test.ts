...
describe('createSurface', () => {
  it('claims touch and selection without taking over layout or placement', () => {
  ...
  it('leaves contextmenu policy to the gesture adapter', () => {
    const canvas = document.createElement('canvas');
    const surface = createSurface(canvas);
  ...
  it('reads rect and size fresh from the canvas', () => {
    ...
    canvas.getBoundingClientRect = () => current;
  ...
  it('hands touch back to the page while not navigable and claims it again after', () => {
    const canvas = document.createElement('canvas');
    canvas.style.touchAction = 'pan-y';
    const surface = createSurface(canvas);

    surface.setNavigable(false);
    expect(canvas.style.touchAction).toBe('pan-y');
    surface.setNavigable(true);
    expect(canvas.style.touchAction).toBe('none');
    surface.destroy();
  });
