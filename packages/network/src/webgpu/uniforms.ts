/**
 * CPU side of the packed uniform buffer shared with `shaders/common/uniforms.wgsl`.
 *
 * Naming: per-item words carry a `v`/`e` prefix; `Px` values are CSS pixels; a bitmask is
 * `<x>.flags` and its bits are `<X>_*`. Channel words map a raw value to a normalized t as
...
export interface CameraRegion {
   ...
   * basis always matches the exact matrix the view-projection was built from.
   */
  setCameraBasis(view: Float32Array): void;
...
/** Display-state uniforms. */
interface DisplayRegion {
  /** `DISPLAY_*` bitmask shared with uniforms.wgsl. */
  flags: number;
}
...
interface GeometryRegion {
  ...
  eDashPeriodPx: number;
  /** World units per unit of normalized height. */
...
interface FocusRegion {
  ...
  /** `FOCUS_*` bitmask shared with uniforms.wgsl. */
  flags: number;
  /** Hover halo opacity. */
  hoverAlpha: number;
  /** Selection halo opacity. */
  ...
  setEndpoints(hoverA: number, hoverB: number, selectedA: number, selectedB: number): void;
}
...
interface ChannelRegion {
  ...
  vColorMin: number;
  /** Vertex color input-domain reciprocal width. */
  vColorScale: number;
  /** Edge color shader mode. */
  eColorMode: number;
  /** Edge color input-domain minimum. */
  eColorMin: number;
  /** Edge color input-domain reciprocal width. */
  eColorScale: number;
  /** Vertex height input-domain minimum. */
  vHeightMin: number;
  /** Vertex height input-domain reciprocal width. */
  vHeightScale: number;
  /** Vertex height output-range minimum. */
  vHeightOutMin: number;
  /** Vertex height output-range span. */
  vHeightOutSpan: number;
  ...
  vSizeMin: number;
  /** Vertex size input-domain reciprocal width. */
  vSizeScale: number;
...
export interface Uniforms {
  ...
  /** Display-state uniform accessors. */
  readonly display: DisplayRegion;
  ...
  readonly vBaseColor: Float32Array;
  /** Resting edge color when the edgeColor channel carries no signal; read only under DISPLAY_EDGE_BASE_COLOR. */
  readonly eBaseColor: Float32Array;
  /** Graticule line color, RGBA in 0..1. */
  readonly graticuleColor: Float32Array;
  /** Tilt ground plane and globe sphere base tone. */
...
export const DISPLAY_GRATICULE = 2;
/**
 ...
 */
export const DISPLAY_GEOGRAPHIC = 4;
/** Display flag bit that paints edges without a color channel in `e_base_color`. */
export const DISPLAY_EDGE_BASE_COLOR = 8;
/** Display flag bit set while the vertex pass draws; edges then end at their endpoint discs. */
export const DISPLAY_VERTICES = 16;
/** Focus flag bit that enables hover/selection rendering. */
export const FOCUS_ENABLED = 1;
/** Focus flag bit that includes selected edge endpoints. */
export const FOCUS_SELECTED_ENDPOINTS = 2;
/** Focus flag bit that includes hovered edge endpoints. */
export const FOCUS_HOVER_ENDPOINTS = 4;
...
type RegionName = 'camera' | 'light' | 'display' | 'frame' | 'geometry' | 'focus' | 'channel';
...
export const UNIFORM_LAYOUT: readonly UniformField[] = [
  { name: 'view_proj', type: 'mat4x4f', word: 0 },
  ...
  { name: 'display_flags', type: 'u32', word: 23, accessors: a('display', 'flags', 'u') },
  ...
  { name: 'v_radius', type: 'f32', word: 30, accessors: a('geometry', 'vRadius') },
  { name: 'e_half_width', type: 'f32', word: 31, accessors: a('geometry', 'eHalfWidth') },
  { name: 'e_dash_period_px', type: 'f32', word: 32, accessors: a('geometry', 'eDashPeriodPx') },
  { name: 'height_amplitude', type: 'f32', word: 33, accessors: a('geometry', 'heightAmplitude') },
  { name: 'v_hover_id', type: 'i32', word: 34, accessors: a('focus', 'vHoverId', 'i'), init: [-1] },
  { name: 'e_hover_id', type: 'i32', word: 35, accessors: a('focus', 'eHoverId', 'i'), init: [-1] },
  {
    name: 'v_selected_id',
    type: 'i32',
    word: 36,
    accessors: a('focus', 'vSelectedId', 'i'),
    init: [-1],
  },
  {
    name: 'e_selected_id',
    type: 'i32',
    word: 37,
    accessors: a('focus', 'eSelectedId', 'i'),
    init: [-1],
  },
  { name: 'v_color_offset', type: 'u32', word: 38, accessors: a('channel', 'vColorOffset', 'u') },
  { name: 'e_color_offset', type: 'u32', word: 39, accessors: a('channel', 'eColorOffset', 'u') },
  { name: 'e_dash_offset', type: 'u32', word: 40, accessors: a('channel', 'eDashOffset', 'u') },
  { name: 'v_height_offset', type: 'u32', word: 41, accessors: a('channel', 'vHeightOffset', 'u') },
  { name: 'v_color_mode', type: 'u32', word: 42, accessors: a('channel', 'vColorMode', 'u') },
  { name: 'v_color_min', type: 'f32', word: 43, accessors: a('channel', 'vColorMin') },
  { name: 'v_color_scale', type: 'f32', word: 44, accessors: a('channel', 'vColorScale') },
  { name: 'e_color_mode', type: 'u32', word: 45, accessors: a('channel', 'eColorMode', 'u') },
  { name: 'e_color_min', type: 'f32', word: 46, accessors: a('channel', 'eColorMin') },
  { name: 'e_color_scale', type: 'f32', word: 47, accessors: a('channel', 'eColorScale') },
  { name: 'v_height_min', type: 'f32', word: 48, accessors: a('channel', 'vHeightMin') },
  { name: 'v_height_scale', type: 'f32', word: 49, accessors: a('channel', 'vHeightScale') },
  { name: 'v_height_mode', type: 'u32', word: 50, accessors: a('channel', 'vHeightMode', 'u') },
  { name: 'v_size_offset', type: 'u32', word: 51, accessors: a('channel', 'vSizeOffset', 'u') },
  { name: 'v_size_mode', type: 'u32', word: 52, accessors: a('channel', 'vSizeMode', 'u') },
  { name: 'v_size_min', type: 'f32', word: 53, accessors: a('channel', 'vSizeMin') },
  { name: 'v_size_scale', type: 'f32', word: 54, accessors: a('channel', 'vSizeScale') },
  { name: 'hover_color', type: 'u32', word: 55, accessors: a('focus', 'hoverColor', 'u') },
  { name: 'selected_color', type: 'u32', word: 56, accessors: a('focus', 'selectedColor', 'u') },
  { name: 'focus_flags', type: 'u32', word: 57, accessors: a('focus', 'flags', 'u') },
  { name: 'hover_alpha', type: 'f32', word: 58, accessors: a('focus', 'hoverAlpha') },
  { name: 'selected_alpha', type: 'f32', word: 59, accessors: a('focus', 'selectedAlpha') },
  { name: 'v_hover_px', type: 'f32', word: 60, accessors: a('focus', 'vHoverPx') },
  { name: 'v_selected_px', type: 'f32', word: 61, accessors: a('focus', 'vSelectedPx') },
  { name: 'e_hover_px', type: 'f32', word: 62, accessors: a('focus', 'eHoverPx') },
  { name: 'e_selected_px', type: 'f32', word: 63, accessors: a('focus', 'eSelectedPx') },
  { name: 'v_height_out_min', type: 'f32', word: 64, accessors: a('channel', 'vHeightOutMin') },
  { name: 'v_height_out_span', type: 'f32', word: 65, accessors: a('channel', 'vHeightOutSpan') },
  { name: 'focus_endpoints', type: 'vec4i', word: 68, init: [-1, -1, -1, -1] },
  { name: 'v_base_color', type: 'vec4f', word: 72 },
  { name: 'graticule_color', type: 'vec4f', word: 76 },
...
export const W_V_RADIUS = wordOf('v_radius');
export const W_E_HALF_WIDTH = wordOf('e_half_width');
export const W_E_DASH_PERIOD_PX = wordOf('e_dash_period_px');
export const W_HEIGHT_AMPLITUDE = wordOf('height_amplitude');
export const W_V_HOVER_ID = wordOf('v_hover_id');
export const W_E_HOVER_ID = wordOf('e_hover_id');
export const W_V_SELECTED_ID = wordOf('v_selected_id');
export const W_E_SELECTED_ID = wordOf('e_selected_id');
export const W_V_HEIGHT_MIN = wordOf('v_height_min');
export const W_V_HEIGHT_SCALE = wordOf('v_height_scale');
...
export const W_V_SIZE_OUT_MIN = wordOf('v_size_out_min');
export const W_V_SIZE_OUT_SPAN = wordOf('v_size_out_span');
export const W_FOCUS_FLAGS = wordOf('focus_flags');
export const W_V_HEIGHT_OUT_MIN = wordOf('v_height_out_min');
export const W_V_HEIGHT_OUT_SPAN = wordOf('v_height_out_span');
export const W_HOVER_ENDPOINT_A = wordOf('focus_endpoints');
...
export function createUniforms(): Uniforms {
  ...
  Object.assign(regions.focus, {
    setEndpoints(hoverA: number, hoverB: number, selectedA: number, selectedB: number) {
  ...
  return {
    ...
    display: regions.display as unknown as DisplayRegion,
    ...
    vBaseColor: vec4View('v_base_color'),
    eBaseColor: vec4View('e_base_color'),
