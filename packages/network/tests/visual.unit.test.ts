...
import { VISUAL, planeHeightAmplitude } from '../src/visual.js';

const bounds: Bounds = { xMin: 0, xMax: 100, yMin: 0, yMax: 50 };

describe('planeHeightAmplitude', () => {
  it('targets a stable pixel height at fit', () => {
    ...
    expect(planeHeightAmplitude(bounds, vp, 0.5)).toBeCloseTo(
  ...
  it('keeps a minimum height budget in vertex radii', () => {
    const scale = planeHeightAmplitude(bounds, { w: 10_000, h: 5_000 }, 1);

    expect(scale).toBeCloseTo(VISUAL.heightMinVertexRadii, 6);
  });

  it('caps height against the graph footprint', () => {
    const scale = planeHeightAmplitude(bounds, { w: 1, h: 1 }, 1);
