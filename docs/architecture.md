...
- One home per type. `Topology`, `Netlist`, `Item`, `Part`, `Series`, `Domain`, `RGBA`, and `Colormap` are defined in `@latkit/model` and imported from there; no renderer re-exports them.
